...
Prescreen-Ratio and Hybrid-Ratio use the multiplicative band
$s(w)/s(w^{*}) \le \tau_{\text{band-ratio}}$ during candidate
screening. They are the strongest genuine defence, pushing
$\mathrm{ASR}@5$ down to 0.341 (Prescreen-Ratio) and 0.293
(Hybrid-Ratio) and lifting $F_{\text{rank}}$ to 0.422 and
0.462 respectively --- close to the ideal middle value of
$0.5$ that characterises a near-uniform ranking. Unlike the GAN
(\Cref{sec:results-gan}), the ratio filters reach low $\mathrm{ASR}$
\emph{and} a central $F_{\text{rank}}$ at the same time, which is what
distinguishes genuine flatness from inverted distinguishability.
...
$75.4\%$ of the time and $F_{\text{rank}}$ is $0.169$ --- the real
...
\emph{Fourth, the GAN's apparent success is a distributional artifact.}
Following the CNN naively, the GAN looks like the flattest method, but
its $F_{\text{rank}}$ near $1$ shows the real password is ranked at the
bottom, so a rational attacker who inverts the ranking reaches
$\mathrm{ASR}@5 = 0.872$ (\Cref{eq:rational}); the same bias is exposed
