...
Add variant uses the additive band condition
$|\, s(w) - s(w^{*}) \,| \le \tau_{\text{band}}$, and the Ratio variant
uses the two-sided multiplicative band condition
$\tfrac{1}{\tau_{\text{band-ratio}}}\, s(w^{*}) \le s(w) \le
\tau_{\text{band-ratio}}\, s(w^{*})$ with $\tau_{\text{band-ratio}} > 1$,
both defined in \Cref{sec:method-filter}.
...
A generative adversarial network trained separately for each
training-set size. Candidates are drawn from the trained generator,
...
instead of masking it at $t/k$. This is the value reported throughout
\Cref{ch:results}; the GAN-focused comparisons in
\Cref{sec:results-gan} additionally break it into its follow-the-CNN and
inverted components, and report the raw per-attacker success, so that
the inversion behind the GAN's number is visible rather than hidden.
